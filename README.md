...
Askora Community Q&A is built following modern WordPress development standards.

*   **Object-Oriented Design:** PSR-4 autoloading via Composer, utilizing Singletons and structured namespaces (`ASKORA\Frontend\Inc\...`).
...
*   **Modular Assets:** CSS is compartmentalized with custom variables (`--askora-primary`) for easy theming. JavaScript uses standard jQuery patterns bound to `$(document).on()` for robust event delegation on dynamically loaded AJAX elements.
...
Embed Askora Community Q&A functionality anywhere on your site using these shortcodes:

*   `[askora_submit_form]` - The frontend question submission form. (Logged-in users only by default).
*   `[askora_questions]` - Lists published questions with AJAX sort/filter controls. Attributes: `category`, `tag`, `orderby`, `per_page`.
*   `[askora_search]` - AJAX live search form. Results appear instantly as you type.
*   `[askora_auth]` - Combined tabbed login + registration form.
*   `[askora_login]` - Standalone phone number + password login form.
*   `[askora_register]` - Standalone phone number registration form.
*   `[askora_popular_questions]` - Shows top-voted questions.
*   `[askora_unanswered_questions]` - Shows questions with zero answers.
*   `[askora_dashboard]` - User dashboard showing their own submitted questions.
...
1.  Upload the `askora-community-qa` folder to your `/wp-content/plugins/` directory.
2.  Activate the plugin through the 'Plugins' menu in WordPress.
3.  Navigate to **Askora -> Settings** in the admin dashboard.
4.  **Set up the Ask Page:** 
    * Create a new WordPress page (e.g., "Ask a Question").
    * Add the `[askora_submit_form]` shortcode to the page.
    * Go to **Askora -> Settings -> General** and select this page from the "Ask a Question Page" dropdown.
...
The architecture includes a framework for the upcoming **Askora Pro** extension. Planned Pro features include:
